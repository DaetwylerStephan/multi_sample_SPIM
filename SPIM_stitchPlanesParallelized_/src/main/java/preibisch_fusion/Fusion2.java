...
import preibisch_imagelib2.interpolation.randomaccess.NearestNeighborInterpolatorFactory;
...
{	//private static String outputdirectory_name; //added St.D funktioniert nicht...uebergebe Dateinamen!
	//public static void setOutputFileName(String name){outputdirectory_name = name;} //added St.D
	 ...
	 * @param T 
	 */
	public static  < T extends RealType< T > & NativeType< T > > ImagePlus fuse(final ArrayList< ImagePlus > images, final ArrayList< InvertibleBoundable > models, 
			final int dimensionality, final boolean subpixelResolution, final int fusionType, final String outputDirectory, final boolean noOverlap, final boolean ignoreZeroValues, final boolean displayImages, String outputdirectory_name)
	{
		...
		// for output
		ImagePlus impType = images.get(0);
		boolean is32bit,is16bit,is8bit;
		T targetType = null;
		
		if ( impType.getType() == ImagePlus.GRAY32 )
			{is32bit = true;
			targetType =  (T) new FloatType();
			}
			else if ( impType.getType() == ImagePlus.GRAY16 )
			{is32bit = true;
			 targetType = (T) new UnsignedShortType();
			}
			else if ( impType.getType() == ImagePlus.GRAY8 )
			{is32bit = true;
			 targetType =  (T) new UnsignedByteType();
			}
			...
			for ( int c = 1; c <= numChannels; ++c )
			{
				...
				if ( subpixelResolution )
				{
					...
					else
					{
						...
						writeBlock( out, numSlices, t, numTimePoints, c, numChannels, blockData, offset, models, fusion, outputDirectory,outputdirectory_name );
					}
				}
				else
				{
	...
	protected static <T extends RealType<T>> void writeBlock( final Img<T> outputSlice, final int numSlices, final int t, final int numTimePoints, final int c, final int numChannels, 
			...
			final ArrayList< InvertibleBoundable > transform, final PixelFusion fusion, final String outputdirectory, final String outputdirectory_name )
	{
		final int numImages = input.size();
		...
		try 
		{
			...
			for ( int slice = 0; slice < numSlices; ++slice )
			{
				...
				IJ.saveAsTiff(outImp, outputdirectory_name);
